...
const HoteltItem = ({ data }: HotelItemProps) => {
  ...
  return (
    <>
      {
        !isLoading && <div className={clsx('flex gap-6 mb-4 last-of-type:mb-0', styles.resultItem)}>
          <div >
            <div className='w-56 h-56 overflow-hidden rounded-xl'>
              <Image className='w-full h-full object-cover' src={data.img} alt={data.name} width={0} height={0} unoptimized />
            </div>
          </div>
          ...
          <div className='flex flex-col justify-around  relative w-full'>
            ...
            <h1 className={clsx(styles.itemTitle, 'line-clamp-1')}>{data.name}</h1>
            ...
            <div className='flex items-center mb-2'>
              ...
              <p className='text-Main text-[12px] leading-[18px] ml-3'>{data.addressLine1}</p>
            </div>

            <p className={clsx(styles.itemContent, 'line-clamp-3')}>{data.description}</p>

            <div className='mt-2'>
            <TagName style={TagNameStyle.default} content={'tag Name'} className='shadow-[0px 0px 2px rgba(0, 0, 0, 0.2)] mr-2 mb-2' />
            </div>

            <div className='text-right w-full'>
              <ButtonShare onClick={() => { router.push('/rooms') }} style={BtnConstClassName.dark} content={'show room'} />
            </div>
          </div>
        </div>
      }
