        ...
        <div className="border-b border-gray-400 border-solid border-lineGray py-4 first-of-type:pt-0">
            <div
                ...
                className={clsx('flex justify-between items-center cursor-pointer', {
                    'mb-4': !showContent && data?.content?.length !== 0,
                })}
            >
                <h4 className="!text-[16px] font-medium ">{data?.title}</h4>
                <UpOutlined
                    className={clsx('text-[14px] transition-all duration-150', {
            ...
            <div className="flex flex-col gap-y-2">
                {showAllLabel
                    ? data?.content?.map((ele: any, index: number) => {
                          const foundIndex = listData.findIndex((val: string) => val === ele.value);
                          return (
                              !showContent && (
                                  <div
                                      key={index}
                                      className="flex justify-between items-center first-of-type:mt-0 text-[16px]"
                                  >
                                      <CheckBoxLabel
